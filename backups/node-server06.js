...
// NOTE: Replaced mySendButton with myChatButton and myPromptButton.
...
<body>
    ...
    <script>
        ...
        // REPLACED: mySendButton -> myChatButton, myPromptButton
        ...
        // NEW: Function to send a normal chat message (Prepends Name: )
        function mySendChatMessage() {
            // Use the utility function to send a normal message
            mySendMessage(false); 
        }
        
        // NEW: Function to send an LLM prompt (Prepends Name: [LLM] )
        function mySendLLMPrompt() {
            // Use the utility function to send an LLM prompt
            mySendMessage(true); 
        }

        // MODIFIED: Accepts a boolean to determine message type
        async function mySendMessage(myIsPrompt) {
            const myMessage = myMessageInput.value.trim();
            if (myMessage === "") { return; }
            
            let myPrefix = '';
            // If it's a prompt, add the [LLM] tag
            if (myIsPrompt) {
                myPrefix = '[LLM] ';
            }
            
            // Prepend the user name (stored in myUserName) and the appropriate prefix
            ...
            if (myWebSocket && myWebSocket.readyState === WebSocket.OPEN) {
                ...
                // Display the user's message locally with the correct prefix visible
        ...
        function myDisplayMessage(myText, myType) {
            ...
            if (myType === 'other') {
                // Use a regex to look for "Name: Message" format
...
myServer.on('connection', function myHandleClientConnect(myClient) {
    ...
    myClient.on('message', async function myHandleIncomingMessage(myMessage) {
        ...
        const myIsLLMPrompt = myMessageString.includes('[LLM]');
        
        // Log the message type
        if (myIsLLMPrompt) {
             console.log(`Received LLM Prompt from student and Broadcasting: ${myMessageString}`);
        } else {
             console.log(`Received Chat Message and Broadcasting: ${myMessageString}`);
        }
        
        // Broadcast the message (which now includes the "Name: [LLM] Message" prefix from the client)
        myServer.clients.forEach(function myBroadcast(myOtherClient) {
            // Check if the client is still open and is NOT the sender
            // Note: The teacher/LLM client will also receive the message to process the prompt
            if (myOtherClient.readyState === myWebSocket.OPEN) {
                // Send to all clients, including the sender, because the teacher/LLM needs to see it,
                // and the sender is already displaying their message locally.
                // However, the original code excluded the sender. We will keep excluding the sender 
                // for simplicity, as the student already displays their message immediately.
                // The teacher's client will need to be configured differently to process ALL messages, 
                // including its own, if it is the LLM client.
                if (myOtherClient !== myClient) {
